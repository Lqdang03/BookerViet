...
//   const [filterActive, setFilterActive] = useState("all"); // New state for active filter

//   useEffect(() => {
//     fetchUsers();
//   }, [searchEmail, filterActive]); // Added filterActive to dependency array
...
//       if (filterActive !== "all") {
//         const isActiveStatus = filterActive == "true";
...
//       // Note the change from user._id to userId in the URL
//       const response = await axios.put(`http://localhost:9999/admin/users/${user._id}/change-status`, {}, {
//         headers: { Authorization: `Bearer ${token}` }
//       });
  
//       // Update the user's status based on the response from the server
//       setUsers(users.map(u =>
//         u._id === user._id ? { ...u, isActivated: response.data.data.isActivated } : u
//       ));
//     } catch (error) {
